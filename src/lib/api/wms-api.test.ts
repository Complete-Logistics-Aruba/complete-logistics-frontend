...
describe("wmsApi - Authentication", () => {
	...
	it("should handle login with invalid credentials", async () => {
		...
		await expect(auth.login("test@example.com", "wrong")).rejects.toThrow("Invalid email or password");
	});
	...
	it("should handle login network error", async () => {
		...
		await expect(auth.login("test@example.com", "password")).rejects.toThrow("Network error");
	});
	...
	it("should handle getCurrentUser error gracefully", async () => {
		const mockSupabase = supabaseClient.supabase as unknown as MockedSupabase;
		mockSupabase.auth.getUser.mockRejectedValueOnce(new Error("Failed to get user"));
...
describe("wmsApi - Products", () => {
	...
	it("should handle product creation error", async () => {
		...
		await expect(products.create(newProduct)).rejects.toThrow("Failed to create product");
	});
	...
	it("should handle product update error", async () => {
		...
		await expect(products.update("invalid-id", { active: false })).rejects.toThrow("Failed to update product");
	});
});
...
describe("wmsApi - Receiving Orders", () => {
	...
	it("should handle receiving order creation error", async () => {
		...
		await expect(receivingOrders.create(newOrder)).rejects.toThrow("Failed to create receiving order");
	});
	...
	it("should handle receiving order fetch error", async () => {
		...
		await expect(receivingOrders.getById("invalid-id")).rejects.toThrow("Failed to load receiving order");
	});
});
...
describe("wmsApi - Pallets", () => {
	...
	it("should handle pallet creation error", async () => {
		...
		await expect(pallets.create(newPallet)).rejects.toThrow("Failed to create pallet");
	});
	...
	it("should handle pallet deletion error", async () => {
		...
		await expect(pallets.delete("invalid-id")).rejects.toThrow("Failed to delete pallet");
	});
});
...
describe("wmsApi - Shipping Orders", () => {
	...
	it("should handle shipping order creation error", async () => {
		...
		await expect(shippingOrders.create(newOrder)).rejects.toThrow("Failed to create shipping order");
	});
});
...
describe("wmsApi - Locations", () => {
	...
	it("should handle location resolution error", async () => {
		...
		await expect(locations.resolve("W1", 1, 1, "A")).rejects.toThrow("Location not found");
	});
});
...
describe("wmsApi - Storage", () => {
	...
	it("should handle file upload error", async () => {
		...
		await expect(storage.upload("bucket", "path/file.pdf", file)).rejects.toThrow("Failed to upload file");
	});
	...
	it("should handle file download error", async () => {
		...
		await expect(storage.download("bucket", "path/file.pdf")).rejects.toThrow("Failed to download file");
	});
	...
	it("should handle file deletion error", async () => {
		...
		await expect(storage.delete("bucket", "path/file.pdf")).rejects.toThrow("Failed to delete file");
	});
});
...
describe("wmsApi - Email", () => {
	...
	it("should validate email inputs", async () => {
		await expect(email.send("", "Subject", "Body")).rejects.toThrow("Email requires to, subject, and body");
	});

	it("should validate email subject", async () => {
		await expect(email.send("test@example.com", "", "Body")).rejects.toThrow("Email requires to, subject, and body");
	});

	it("should validate email body", async () => {
		await expect(email.send("test@example.com", "Subject", "")).rejects.toThrow("Email requires to, subject, and body");
	});
});
	...
	describe("wmsApi - Locations", () => {
		...
		describe("AC: 3 - Error handling", () => {
			it("should throw error when location not found (AC: 3)", async () => {
				...
				await expect(locations.resolve(mockWarehouseId, 9, 5, "Z")).rejects.toThrow("Location not found");
			});
