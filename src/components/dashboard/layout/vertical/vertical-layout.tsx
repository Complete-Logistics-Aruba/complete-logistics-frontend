...
export function VerticalLayout({ children }: VerticalLayoutProps): React.JSX.Element {
	...
	return (
		<React.Fragment>
			...
			>
				<RoleBasedNavigation navItems={dashboardConfig.navItems}>
					{(filteredItems) => <SideNav color={navColor} items={filteredItems} />}
				</RoleBasedNavigation>
				<Box sx={{ display: "flex", flex: "1 1 auto", flexDirection: "column", pl: { lg: "var(--SideNav-width)" } }}>
					<RoleBasedNavigation navItems={dashboardConfig.navItems}>
						{(filteredItems) => <MainNav items={filteredItems} />}
					</RoleBasedNavigation>
					<Box
						component="main"
						sx={{
