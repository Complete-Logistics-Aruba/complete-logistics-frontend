			...
			<Stack spacing={2} sx={{ p: 2 }}>
				<div>
					<Box component={RouterLink} href={paths.home} sx={{ display: "inline-flex" }}>
						<Logo color={logoColor} height={80} width={160} />
					</Box>
				</div>
				<WorkspacesSwitch />
			</Stack>
...
function renderNavItems({
	...
	const children = items.reduce((acc: React.ReactNode[], curr: NavItemConfig): React.ReactNode[] => {
		...
		// Check if any nested child item at any depth is active for smarter menu expansion
		const isAnyChildActive = (items?: NavItemConfig[]): boolean => {
			if (!items) return false;

			return items.some((item) => {
				// Check if current item is active
				const isCurrentActive = item.href && pathname.startsWith(item.href);
				// Recursively check children
				const isChildActive = isAnyChildActive(item.items);
				return isCurrentActive || isChildActive;
			});
		};

		// Force open if this item or any of its children at any depth is active
		const forceOpen = childItems ? isAnyChildActive(childItems) : false;
...
function NavItem({
	...
	return (
		<Box component="li" data-depth={depth} sx={{ userSelect: "none" }}>
			...
			{showChildren ? (
				<div style={{ paddingLeft: "12px", borderLeft: "1px solid var(--NavItem-children-border)" }}>{children}</div>
			) : null}
		</Box>
	);
}
